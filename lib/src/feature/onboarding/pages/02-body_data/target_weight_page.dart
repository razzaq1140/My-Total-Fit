    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: const PreferredSize(
          ...
          child: CustomProgressBar(currentStep: 5, totalSteps: 10),
        ),
      ),
          ...
          children: [
                ...
                children: [
                  ...
                  Consumer<OnboardingProvider>(
                    builder: (context, provider, child) {
                      ...
                      // .toStringAsFixed(1);
                      final double weight =
                          (double.parse(provider.currentWeight) -
                              selectedWeight);
                      ...
                      return RichText(
                        text: TextSpan(
                          ...
                          children: [
                            ...
                            TextSpan(text: weight > 0 ? " lose " : " gain "),
                            TextSpan(
                              text: weight.abs().toStringAsFixed(1),
                              style: textTheme(context).bodyMedium?.copyWith(
                                  color: colorScheme(context).primary,
                                  fontWeight: FontWeight.bold),
                            ),
                            const TextSpan(
                                text: " body weight Moderate\nweight"),
                            TextSpan(text: weight > 0 ? " lose " : " gain "),
                            const TextSpan(
                                text:
                                    "can also make a big difference:\n • Lower blood pressure\n • Reduce the risk of type 2 disbetes"),
                          ],
                        ),
                      );
                    },
            ...
            Consumer<OnboardingProvider>(
              builder: (context, provider, child) => CustomAnimatedButton(
                onTap: () {
                  provider.updateTargetWeight(
                      weight: selectedWeight.toStringAsFixed(1));
                  provider.updateTargetWeightText(
                    weightTxt: isKg ? 'kg' : 'lbs',
                  );
                  context.pushNamed(AppRoute.bmiDetails);
                },
              ),
            ),
            const SizedBox(height: 30),
